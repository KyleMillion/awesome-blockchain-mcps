...
- **[The Stall](https://the-stall.intuitek.ai/mcp)** – **172 pay-per-call** blockchain analytics and financial intelligence tools. Covers on-chain analytics (EVM logs, wallet scoring, ERC-20 snapshots, token security), DeFi metrics (whale radar, protocol revenue, staking yields, liquidity pools), crypto prices/market data, and compliance (OFAC sanctions screening). Payments via **x402 protocol** (USDC on Base) — no API key, sub-cent per call.
